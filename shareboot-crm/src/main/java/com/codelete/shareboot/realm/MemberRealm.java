package com.codelete.shareboot.realm;
...
import com.codelete.shareboot.service.impl.MemberServiceImpl;
import com.codelete.shareboot.util.PasswordUtil;
...
public class MemberRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		Member member = memberServiceImpl.get(mid) ; // 通过用户名获取用户信息
		...
		if (member.getLocked().equals(1)) { // 1表示非0，非0就是true
			throw new LockedAccountException("用户锁定中！");
		}
		...
		SecurityUtils.getSubject().getSession().setAttribute("user", member.getName());
		return auth;
	}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		Map<String, Set<String>> map = memberServiceImpl.listAuthByMember(mid) ;
